import { UpdateOption } from "./Option"
...
export const Update = <T extends object>(tableName: string, updata: Partial<T>, quest: UpdateOption<T> = {}) => {
    ...
    /** 排序 */
    if (order !== undefined) sqlContainer.push(`ORDER BY ${order[0]} ${order[1]}`)
    if (limit !== undefined) sqlContainer.push(`LIMIT ${limit}`)
